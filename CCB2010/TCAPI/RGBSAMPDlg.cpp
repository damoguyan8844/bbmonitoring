...
extern long g_VGACAP_Framerate;
extern string g_latestCapture;
extern volatile long g_latestCaptureIndex;


extern LPVOID g_Capture_Buffer1;
extern int g_Capture_Buffer1_Width;
extern int g_Capture_Buffer1_Height;

extern LPVOID g_Capture_Buffer2;
extern int g_Capture_Buffer2_Width;
extern int g_Capture_Buffer2_Height;

extern volatile bool   g_Capture_Buffer_UseAble;
extern volatile bool   g_Capture_Use_Buffer1;
extern volatile int	   g_Capture_Index;
...
//	HDC hDC;
	...
	{
		LogMessage("Thread_paint  Has Exception",LOG_ERROR);
	}
...
CRGBSAMPDlg::CRGBSAMPDlg(HWND * pParent /*=NULL*/,HWND hWnd)
{
...
BOOL CRGBSAMPDlg::InitDevice()
{
	...
	if (0 == ulNumber)
	{
		LogMessage("没有设备",MB_ICONWARNING | MB_OK);
		return(FALSE);
	}
	return(TRUE);
}
...
// 	delete cFont;
	...
	hRGBCapture1 = 0;
	ActiveStatic = 0;   //?????????:0-?????;1-??????
    PauseRunSignal1 = false;    //????????:true??,false??
	bUsingAccBuffer1=false;  //????????????,true?;false?
    bufferSize1 = 0L;         //?1???????????
	startcount1 = FALSE;
	g_framecount1 = 0;  //??????????????
	signalflag = FALSE;
...
// 		{
// 			//??? menuPopup ??????
// 			menuPopup.AppendMenu(MF_STRING, ID_Run_VGA1, "??VGA");
// 			menuPopup.AppendMenu(MF_STRING, ID_Stop_VGA1, "??VGA");
// 			menuPopup.AppendMenu(MF_SEPARATOR);
// 			menuPopup.AppendMenu(MF_STRING, ID_ADJUST_VGA1, "VGA????");
// 			menuPopup.AppendMenu(MF_STRING, ID_QUALPROP_VGA1, "????");
// 			menuPopup.AppendMenu(MF_SEPARATOR);
// 			menuPopup.AppendMenu(MF_STRING, ID_PAUSE_VGA1, "??VGA");
...
void CRGBSAMPDlg::OnRunVGA1() 
{
	...
	{
		LogMessage("VGA1采集卡已运行 !系统提示",MB_ICONWARNING | MB_OK);
	}
}
...
//	*pColourMask=16711680;
//	return RGBPIXELFORMAT_888;
...
//	if ( MessageBox("???????? ?\t","????",MB_ICONQUESTION | MB_OKCANCEL ) != IDOK )
	{
		return ;
	}
//	CDialog::OnClose();
...
{ 
	try
	{
		...
		if(g_Capture_Buffer_UseAble)
		{
			...
			if(true)
			{
				...
				{
					g_Capture_Buffer1_Width=width;
					g_Capture_Buffer1_Height=height;
					data_size=g_Capture_Buffer1_Width*g_Capture_Buffer1_Height*3;
					pBuffer = & g_Capture_Buffer1;
				}
				else
				{
					g_Capture_Buffer2_Width=width;
					g_Capture_Buffer2_Height=height;
					
					data_size=g_Capture_Buffer2_Width*g_Capture_Buffer2_Height*3;
					pBuffer = & g_Capture_Buffer2;
				}
				g_Capture_Use_Buffer1=!g_Capture_Use_Buffer1;
			}
			memcpy((char *)(*pBuffer),(const unsigned char *)captureBuffer.LpVoidBuffer,data_size);
			g_Capture_Index++;
		...
		else
		{
			...
			static int lastCaptureIndex=0;
			char bmpName[128];
			sprintf(bmpName,"%s\\%d_%d_%d_%03d.bmp",m_WorkFolder.c_str(),lastCaptureIndex++ % m_maxCachePicNumber,width,height,g_latestCaptureIndex % m_maxCachePicNumber);
			
			if (1 == datamode)
			{
				unsigned char * pTemp=(unsigned char *)captureBuffer.LpVoidBuffer;
				RGB24WriteBmp(width,height,pTemp+0,pTemp+1,pTemp+2,1600*1200*4,bmpName);
			}
			
			if(true)
			{
				Lock lock(&_cs);
				g_latestCapture=bmpName;
			}
		}
	...
	{
		LogMessage("Paint Frame Come To Error!",LOG_ERROR);
	}
}
...
{
	string & strInfo=singnalInfo;
	switch(wParam)
	{
		case NOSIGNAL_NO_RESOLUTION:
		{
			strInfo = string("无 VGA 信 号(错误的分辨率 !!!)");
			break;
		}
		case NOSIGNAL_DDRCHECK_ERROR:
		{
			strInfo = string("无 VGA 信 号(DDR 自检出错 !!!)");
			break;
		}
		case NOSIGNAL_H_OVERFLOW:
		{
			strInfo = string("无 VGA 信 号(水平偏移量溢出 !!!)");
			break;
		}
		case NOSIGNAL_V_OVERFLOW:
		{
			strInfo = string("无 VGA 信 号(垂直偏移量溢出 !!!)");
			break;
		}
		default:
		{
